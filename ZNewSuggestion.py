    ...
    def classification(self, input, reuse = tf.AUTO_REUSE) :
        ...
        with tf.name_scope("classification") :          
          ...
          with tf.variable_scope("FullyConnected1") as scope:
              net = tf.contrib.layers.fully_connected(input,2,reuse=reuse,activation_fn=tf.nn.relu, biases_initializer=tf.zeros_initializer(), scope = 'FullyConnected1')
          ...
          with tf.variable_scope("FullyConnected2") as scope:
              net = tf.contrib.layers.fully_connected(net,2,reuse=reuse,activation_fn=tf.nn.relu , biases_initializer=tf.zeros_initializer(),  scope = 'FullyConnected2')
              weights_initializer = tf.contrib.layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32)        
          
          
          with tf.variable_scope("FullyConnected3") as scope:
              net = tf.contrib.layers.fully_connected(net,2,reuse=reuse,activation_fn=tf.nn.relu, biases_initializer=tf.zeros_initializer(), scope = 'FullyConnected3')
              weights_initializer = tf.contrib.layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32)        
